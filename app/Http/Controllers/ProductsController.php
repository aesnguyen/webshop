...
class ProductsController extends Controller
{
    ...
    public function getadd($id)
    {
		...
		$pro = Products::all();	
        if ($p_id >=19) {
                return view('back-end.products.items-add',['data'=>$pro,'cat'=>$cat,'loai'=>$p_name->name]);
            }
        else {
	...
	return redirect('admin/sanpham/all')
      ->with(['flash_level'=>'result_msg','flash_massage'=>' Add successfully !']);    	
    ...
    public function getdel($id)
    {
        ...
        $pro->delete();
        return redirect('admin/sanpham/all')
         ->with(['flash_level'=>'result_msg','flash_massage'=>'Removed !']);
    }
    public function getedit($loai,$id)
    {
    	...
    	if ($p_id == 1) {
            ...
            return view('back-end.products.edit-sneaker',['pro'=>$pro,'cat'=>$cat,'loai'=>'Điện thoại']);    
        } elseif ($p_id ==2) {
            $cat= Category::where('parent_id', 2)->get();
            $pro = Products::where('id',$id)->first();
            return view('back-end.products.edit-sneaker',['pro'=>$pro,'cat'=>$cat,'loai'=>'Laptop']);       
        } elseif ($p_id ==19) {
            $cat= Category::where('parent_id', 19)->get();
            $pro = Products::where('id',$id)->first();
            return view('back-end.products.edit-sneaker',['pro'=>$pro,'cat'=>$cat,'loai'=>$p_id]);     
        }
    }
