...
class OdersController extends Controller
{
    ...
    public function postdetail($id)
    {
    	...
    	return redirect('admin/donhang')
      	->with(['flash_level'=>'result_msg','flash_massage'=>' Confirmed order successfully !']);    	
    ...
    {       
    	...
    	if ($oder->status ==1) {
    		...
    		->with(['flash_level'=>'result_msg','flash_massage'=>'Cannot cancel order: '.$id.' cause be confirmed!']);
    	} else {
    		$oder = Oders::find($id);
        	$oder->delete();
        	return redirect('admin/donhang')
         	->with(['flash_level'=>'result_msg','flash_massage'=>'Canceled order:  '.$id.' !']);
     	}
    }
}
